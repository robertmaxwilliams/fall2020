...
void sequential_all_pairs(
    std::vector<value_t>& mnist,		// mXn matrix
    std::vector<value_t>& all_pair,		// mXm matrix about 16GB space	
    index_t rows,						// m
    index_t cols)						//n
{
...
int all_pairs() 
{
    ...
    load_binary(mnist.data(), rows*cols, ".data/X.bin"); 
	//load_binary(mnist.data(), rows*cols, "./data/mnist_65000_28_28_32.bin");
	TIMERSTOP(load_data_from_disk)

	std::vector<value_t> all_pair(rows*rows); // 65,000 X 65,000 X 4 bytes about 16 GB

	//TIMERSTART(seq_compute_distances)
	//sequential_all_pairs(mnist, all_pair, rows, cols);
	//TIMERSTOP(seq_compute_distances)

    //TIMERSTART(para_compute_distances)
	//dynamic_all_pairs(mnist, all_pair, rows, cols);
    //TIMERSTOP(para_compute_distances)
/*
